...
#define RAMDISK_SIZE ((&ramdisk_end) - (&ramdisk_start) - 8)
...
size_t ramdisk_read(void *buf, size_t offset, size_t len) {
  ...
  memcpy(buf, &ramdisk_start + 8 + offset, len);
  return len;
}
...
size_t ramdisk_write(const void *buf, size_t offset, size_t len) {
  assert(offset + len <= RAMDISK_SIZE);
  memcpy(&ramdisk_start + 8 + offset, buf, len);
  return len;
}

void init_ramdisk() {
  Log("ramdisk info: start = %p, end = %p, size = %d bytes",
      &ramdisk_start + 8, &ramdisk_end, RAMDISK_SIZE);
}
